...
export default {
  ...
  methods: {
    async runModel () {
      const result = await faceAPI.detectAllFaces(this.$refs.camera, await new faceAPI.SsdMobilenetv1Options({ minConfidence: 0.1 })).withAgeAndGender()
      if (result) {
        const dims = faceAPI.matchDimensions(this.$refs.canvas, this.$refs.canvas, true)
        const resizeResults = faceAPI.resizeResults(result, dims)
        // await faceAPI.draw.drawDetections(this.$refs.canvas, resizeResults)
        if (resizeResults.length > 0) {
          this.isFaceRecognized = true
    ...
    toggleCamera () {
      if (this.isCameraOpen) {
        ...
        this.interval = setInterval(() => this.runModel())
      }
    },
